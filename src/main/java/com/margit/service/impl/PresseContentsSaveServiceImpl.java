...
@Service
public class PresseContentsSaveServiceImpl implements PresseContentsSaveService{
	@Autowired
	...
	public PresseContents savePresse(TextContentsData textContentsData) {
		PresseContents presseContents = new PresseContents();
		if(textContentsData.getTitleImgFile() != null) {
			...
			presseContents.setTitlePhotoId(TextImgFile.getId());
		}
		...
		if(textContentsData.getOrderNo() != 0) {
			...
			orderNo = presseContentsDao.getCurrentOderNo();
			if(orderNo == null) {
				orderNo = 1;
			}else {
				orderNo++;
		...
		if(textContentsData.getId() != 0) {
			presseContents.setId(textContentsData.getId());
		}
		presseContents.setTitle(textContentsData.getTitle());
		presseContents.setContentText(textContentsData.getTextContent());
		presseContents.setOrderNo(orderNo);
		
		PresseContents currentPresseContents = presseContentsDao.save(presseContents);
		
		return currentPresseContents;
	}
